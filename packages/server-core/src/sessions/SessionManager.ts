...
import { type AgentEvent, setPermissionMode, setOrchestratorEnabled as setModeOrchestratorEnabled, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, registerSessionScopedToolCallbacks, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary } from '@craft-agent/shared/agent'
import {
...
import { listLabels, isValidLabelId, loadLabelConfig } from '@craft-agent/shared/labels/storage'
...
// messageToStored and storedToMessage are imported from @craft-agent/core/types
...
export class SessionManager implements ISessionManager {
  ...
  private broadcastProjectsChanged(workspaceId: string): void {
    ...
    this.eventSink(RPC_CHANNELS.projects.CHANGED, { to: 'workspace', workspaceId }, workspaceId)
  }
   ...
   */
  async createSubSession(workspaceId: string, parentSessionId: string, options?: import('@craft-agent/shared/protocol').CreateSessionOptions): Promise<Session> {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) {
      throw new Error(`Workspace ${workspaceId} not found`)
    }
   ...
   */
  getSessionFamily(sessionId: string): { parent: SessionMetadata; siblings: SessionMetadata[]; self: SessionMetadata } | null {
    const managed = this.sessions.get(sessionId)
    if (!managed) return null
